...
use ApiV2\Framework\ISellerQuoteTransformer;
...
class LCLSellerQuoteTransformer implements ISellerQuoteTransformer
{
    public function ui2bo_save($payload, $leadType)
    {
        ...
        if ($leadType == "term") {
            ...
            $post = $serializer->deserialize($payload, 'array<Api\Modules\LCL\LCLSellerQuoteTermBO>', 'json');
        } else {
            //Delegate request to SpotSellerPostService
            $post = $serializer->deserialize($payload, 'Api\Modules\LCL\LCLSellerQuoteSpotBO', 'json');
        }
        ...
        foreach ($rankGrouped as $key => $quote) {
            foreach ($quote as $items) {
                $sellerQuoteArray["attributes"]['l' . $key][] = array(
                    ...
                    'status' => $items["status"],
                    'containers' => $items['attributes']->containers,//$this->getContainersList($items, $sellerQuoteArray),//
