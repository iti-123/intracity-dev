...
class LCLSellerPostIndexer
{
    ...
    private function flattenPortPair(array $index, LCLSellerPortPair $portPair, array &$sellerPostIndexes, array $globalDiscounts = [], $ispublic = false)
    {
    ...
    private function flattenCarrier(array $index, Carriers $carrier, array &$sellerPostIndexes)
    {
        ...
        $fctotal = 0;
        if (count($carrier->freightCharges) > 0) {
            foreach ($carrier->freightCharges as $freightCharge) {
        ...
        if (count($carrier->localCharges) > 0) {
            foreach ($carrier->localCharges as $localCharge) {
    ...
    private function generateSearchQuery(LCLSellerPostSearchBO $bo)
    {
        ...
        $fq = "entity:sellerpost AND service:" . $service;
    ...
    public function generatePostMasterQuery(LCLSellerPostMasterOutboundBO $bo)
    {
