...
use ApiV2\Requests\UserRequest;
use ApiV2\Transformers\AuthTransformer;
...
class AuthController extends BaseController
{
      ...
      public function authenticate(Request $request)
    {
       ...
       // Log::info("Login authenticate Success");
        ...
        $sellerDetailsSql = "SELECT count(*) as count FROM seller_details where user_id = $user->id";
        $sellerDetails = DB::select(DB::raw($sellerDetailsSql));
        $sellerDetailsExists = $sellerDetails[0]->count ? 1 : 0;
        if(!$sellerDetailsExists){
            $user->secondary_role_id = 0;
        }
        ...
        if(isset($user->secondary_role_id) && $user->secondary_role_id != '' && $user->secondary_role_id == '2' && $sellerDetailsExists) {
            $s_role = "Seller";
        }
        ...
        if($current_role == 'Buyer'){
            ...
            $customClaims= [
                ...
                'roleId' => $user->lkp_role_id,
                'role' => $current_role ,
                'active_role_id' => $user->lkp_role_id,
                'active_role_name' => $current_role,
                'primary_role_id' => $user->primary_role_id,
                'primary_role_name' => $p_role,
                'secondary_role_id' => $user->secondary_role_id,
                'secondary_role_name' => $s_role,
                "id" => $user->id,
        ...
        if($current_role == 'Seller'){
            ...
            $role = "Seller";
            $customClaims= ['isAdmin' => $user->is_admin, 'isSuperAdmin' => $user->is_superadmin, 'roleId' => $user->lkp_role_id,'role' => $current_role ,'active_role_id' => $user->lkp_role_id,'active_role_name' => $current_role, 'primary_role_name' => $role,'primary_role_id' => $user->primary_role_id,'secondary_role_id' => $user->secondary_role_id,'secondary_role_name' => $s_role,"id" => $user->id, "firstname" => $user->username, "email" => $user->email, "phone" => $user->phone, "designation" => $designation, "sIds" => $serviceIds];
        }

        if($userRole != '' && $userRole == '3'){ $role = "ADMIN"; }
