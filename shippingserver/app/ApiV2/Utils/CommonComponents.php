 ...
 */
    ...
    public static function sellerBuyerServiceid($userid,$roleId) 
    {
        ...
        try {
            if($roleId==2){
                ...
                $buyerquoteid = DB::table('seller_services')
                                ->select(array( DB::raw('GROUP_CONCAT(DISTINCT lkp_service_id) as Serviceid')))
                                ->where('user_id','=', $userid)
                                ->where('is_service_offered','=',1)->get(); 

            return ($buyerquoteid);                     
                
            }else if ($roleId==1) {
                $count_n = DB::table('seller_services')->select('id')->where('user_id', '=', $userid)
                            ->where('is_service_required', '=', 1)->get();
                if(count($count_n)==0) {
                     ##### for buyer #####
                     $buyerquoteid = DB::table('lkp_services')->select(
                          // 'id as Serviceid'
                          array(DB::raw('GROUP_CONCAT(DISTINCT id) as   Serviceid'))
                     ...
                     )->where('is_service_required', '=', 1)->where('user_id', '=', $userId)->get();
