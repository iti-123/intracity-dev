...
//           CommonComponent::activityLog("GET_STATE", GET_STATE, 0, HTTP_REFERRER, CURRENT_URL);
    ...
    public static function listMessages($request = null, $message_types = GENERALMESSAGETYPE, $receiverId = null, $id = null, $term = 0, $isquery = false, $params = array())
    {
        ...
        try {
            ...
            if ($message_types == ORDERMESSAGETYPE) {
                ...
                } elseif ($serviceId == WAREHOUSE) {
                    if ($roleId == BUYER) {
                        $getAllMessagesQuery->where('um.enquiry_id', $id);
                    } elseif ($roleId == SELLER) {
                        $matchedIds = array();
                        $getpostItems = DB::table('wh_seller_posts as sp')
                            ->leftjoin('wh_seller_post_items as spi', 'spi.seller_post_id', '=', 'sp.id')
                            ->where('sp.id', '=', $id)->select('spi.id')->get();
                        //echo $id;print_r($getpostItems);exit;
                        foreach ($getpostItems as $getpostItem) {
                            $matchedIds[] = $getpostItem->id;
            ...
            /* Start : Private Buyer Condition*/
            if (isset($params['privateBuyer']) && $params['privateBuyer']) {
                $getAllMessagesQuery->where('um.sender_id', $params['privateBuyer']);
            }
            /* End : Private Buyer Condition*/

            if ($isquery == true) {
                return $getAllMessagesQuery;
            }
            ...
            $grid->add('message_type', 'Type', 'message_type')->style('display:none');
            $grid->add('username', 'Name', 'username')->attributes(array("class" => "col-md-2 padding-left-none"));

            $grid->add('created_at', 'Date', 'created_at')->attributes(array("class" => "col-md-2 padding-left-none"));


            $grid->add('subject', 'Subject', 'subject')->attributes(array("class" => "col-md-2 padding-left-none"));

            $grid->add('is_term', '', 'is_term')->style('display:none');
            ...
            $grid->row(function ($row) {
                ...
                $row->cells [1]->attributes(array("style" => 'display:none'));
                ...
                $row->cells [5]->attributes(array("class" => "col-md-2 padding-left-none pull-right"));

                if ($isread == 0) {
                    $row->cells[2]->value = '<div style="width:100%"><div class="table-row "><div class="col-md-2 padding-left-none"><b>
                ' . $uname . '</b>';
                } else {
                    $row->cells[2]->value = '<div style="width:100%"><div class="table-row "><div class="col-md-2 padding-left-none">' . $uname;
                }

                if ($isread == 0) {
                    $row->cells [4]->value = '<b>' . $subject . '</b>';
                } else {
                    $row->cells [4]->value = $subject;
                }

                if ($row->cells [3]->value == "0000-00-00 00:00:00")
                    if ($isread == 1) {
                        $row->cells [3]->value = "NA";
                    } else {
                        $row->cells [3]->value = 'NA';
                    }
                else
                    if ($isread == 1) {
                        $row->cells [3]->value = CommonComponent::checkAndGetDateTime($row->cells [3]->value);
                    } else {
                        $row->cells [3]->value = '<b>' . CommonComponent::checkAndGetDateTime($row->cells [3]->value) . '</b>';
                    }
                $row->cells [4]->value .= "</a>";

                if ($isread == 1) {
                    $row->cells [5]->value = '<a href="#" class="new_message" data-userid="' . $sender_id . '" data-subject="' . $subject . '" data-msgid="' . $msg_id . '"><button class="btn red-btn pull-right submit-data">Reply</button></a></div></div></div>';
                } else {
                    $row->cells [5]->value = '<a href="#" class="new_message" data-userid="' . $sender_id . '" data-subject="' . $subject . '" data-msgid="' . $msg_id . '"><button class="btn red-btn pull-right submit-data">Reply</button></a></div></div></div>';
                }
        ...
        try {
            ...
            switch ($serviceId) {
                ...
                case ROAD_TRUCK_LEASE :
                    ...
                    break;
                case WAREHOUSE :
                    $randString = 'WAREHOUSE/' . $created_year . '/' . str_pad($msgid, 6, "0", STR_PAD_LEFT);
                    $message_no = $randString;
                    break;
                default:
...
//            }
    ...
    public static function generateBuyerMessagesMasterGrid($buildQuery, $messageMasterType)
    {
        self::$messageMasterType = $messageMasterType;
        $resultQuery = Datatables::of($buildQuery);

        $resultQuery->addColumn('username', function ($query_data) {
            return $query_data->username;
        });
        $resultQuery->editColumn('created_at', function ($query_data) {
            return $query_data->created_at;
        });
        $resultQuery->editColumn('subject', function ($query_data) {
            return $query_data->subject;
        });
        $resultQuery->addColumn('reply', function ($query_data) {
            return '<a href="#" class="new_message" data-userid="' . $query_data->sender_id . '" data-subject="' . $query_data->subject . '" data-msgid="' . $query_data->id . '"><button class="btn red-btn pull-right submit-data">Reply</button></a>';
        });

        $resultQuery->addColumn('redirectURL', function ($query_data) {
            if (self::$messageMasterType == 'in')
                $out = 1;
            else
                $out = 0;

            $redirectURL = '/getmessagedetails/' . $query_data->id . '/0/' . $out;
            return $redirectURL;
        });

        return $resultQuery->make(true);
