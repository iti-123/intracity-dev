...
class MasterLocationData
{
    ...
    {
        try {
            return $queries = DB::table('buyer_details')
                                ->join('users', 'buyer_details.user_id', '=', 'users.id')
                                ->select('buyer_details.user_id as id', DB::raw('CONCAT(firstname, " ", lastname) AS full_name'))
                                ->where('users.is_active', 1)
                                ->orderBy('buyer_details.id', 'DESC')
                                ->get();
        } catch (Exception $e) {
            echo 'Caught exception: ', $e->getMessage(), "\n";
        }
