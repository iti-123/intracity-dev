...
class IntraHyperBuyerPostRequest
{
    ...
    public static function getTermTitle($value)
    {
        $title = '';
        if (!empty(self::has($value, 'term_distance_title'))) {
            $title = $value->term_distance_title;
        } else if (!empty(self::has($value, 'title_hour'))) {
            $title = $value->title_hour;
        }  
