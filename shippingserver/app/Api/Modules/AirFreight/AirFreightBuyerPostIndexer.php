...
class AirFreightBuyerPostIndexer implements IBuyerPostIndexer
{
    ...
    public function rebuildIndex(BuyerPostBO $bo)
    {
        ...
        try {
            ...
            $fclSearchRec->serviceName = "AirFreight";
    ...
    function fillSpotRoutes(AirFreightBuyerPostAttributes $attributes, FCLBuyerPostIndex $airFreightSearchRec)
    {

        $buyerPostIndex = array();
        $now = date('Y-m-d H:i:s');

        LOG::info("Filling routing information");
        if (!empty($attributes)) {
            $route = $attributes->route;
            $airFreightSearchRec->loadPort = $route->loadPort;
            $airFreightSearchRec->dischargePort = $route->dischargePort;
            $airFreightSearchRec->serviceSubType = $route->serviceSubType;
            $airFreightSearchRec->originLocation = $route->originLocation;
            $airFreightSearchRec->destinationLocation = $route->destinationLocation;
            $airFreightSearchRec->commodity = $route->commodity;
            $airFreightSearchRec->cargoReadyDate = $route->cargoReadyDate;
            $airFreightSearchRec->priceType = $route->priceType;
            $airFreightSearchRec->airFreightType = $route->airFreightType;
            //As there is no search on Package dimensions the dimensions parameters need not be inserted into Solr.
            //$this->fillPackageDimentions($route, $airFreightSearchRec);
            $airFreightSearchRec->created_at = $now;
            $airFreightSearchRec->updated_at = $now;
            $buyerPostIndex[] = json_decode($airFreightSearchRec, true);
        }

        FCLBuyerPostIndex::insert($buyerPostIndex);

    }

    function fillTermServiceType(AirFreightBuyerPostAttributes $attributes, FCLBuyerPostIndex $fclSearchRec)
    {
    ...
    function fillTermRoutes(AirFreightServiceType $service, FCLBuyerPostIndex $airFreightSearchRec)
    {
    ...
    function fillPackageDimentions(Route $routeBo, FCLBuyerPostIndex $airFreightSearchRec)
    {
        LOG::info("Filling packageDimensions information");
        $fclBuyerPostIndex = array();
        $now = date('Y-m-d H:i:s');
        if (!empty($routeBo)) {

            //TODO  Get an array later
            $pkgDimensions [] = $routeBo->packageDimensions;

            LOG::info("Listing out Package Dimenstions *******************");
            foreach ($pkgDimensions as $pkgDimension) {

                LOG::info((array)$pkgDimension);
                LOG::info((array)$airFreightSearchRec);

                $airFreightSearchRec->airFreightType = $routeBo->airFreightType;
                $airFreightSearchRec->length = $pkgDimension->length;
                $airFreightSearchRec->breadth = $pkgDimension->breadth;
                $airFreightSearchRec->height = $pkgDimension->height;
                $airFreightSearchRec->lbhUnit = $pkgDimension->lbhUnit;
                $airFreightSearchRec->noOfPackages = $pkgDimension->noOfPackages;
                $airFreightSearchRec->totalCBM = $pkgDimension->totalCBM;
                $airFreightSearchRec->grossWeight = $pkgDimension->grossWeight;
                $airFreightSearchRec->weightUnit = $pkgDimension->weightUnit;
                $airFreightSearchRec->created_at = $now;
                $airFreightSearchRec->updated_at = $now;
                ...
                $fclBuyerPostIndex[] = json_decode($airFreightSearchRec, true);
            }
    ...
    /*
        function fillTermContainers(TermRoute $routeBo, FCLBuyerPostIndex $fclSearchRec) {
            ...
            if (!empty($routeBo)) {
                ...
                $fclContainers = $routeBo->packageDimensions;
                foreach ($fclContainers as $pkgDimension ) {
                    $fclSearchRec->length  = $pkgDimension->length;
                    $fclSearchRec->breadth = $pkgDimension->breadth;
                    $fclSearchRec->height = $pkgDimension->height;
                    $fclSearchRec->lbhUnit  = $pkgDimension->lbhUnit;
                    $fclSearchRec->noOfPackages = $pkgDimension->noOfPackages;
                    $fclSearchRec->totalCBM = $pkgDimension->totalCBM;
                    $fclSearchRec->grossWeight = $pkgDimension->grossWeight;
                    $fclSearchRec->weightUnit = $pkgDimension->weightUnit;
    ...
    private function generateSearchQuery(FCLBuyerPostSearchBO $bo)
    {
        ...
        $fq = "entity:buyerpost AND service:" . AirFreight;
    ...
    public function addAditionalDataForSearch($jsonResponse)
    {
        ...
        for ($i = 0; $i < sizeof($docs); $i++) {
            $docs[$i]->sellerQuoteStatus = $this->getSellerQuoteStauts($docs[$i]->postId);
    ...
    public function getSellerQuoteStauts($postId = '')
    {
        ...
        if (sizeof($sellerQuoteStatus) > 0) {
            return $sellerQuoteStatus[0]['status'];
        } else
            return '';
    }
    ...
    {
        $userId = JWTAuth::parseToken()->getPayload()->get('id');
        $fq = "entity:buyerpost AND service:" . AirFreight . " AND";
