...
use Api\Modules\Common\SettingService;
use Api\Services\CacheControlService;
use Api\Services\NotificationService;
use DB;
use Log;

class FCLBuyerSellerPostRecommender //implements IBuyerSellerPostRecommender
{
    ...
    private function _filterSellerInboundPostMaster($sellerId, FCLSellerPostMasterInboundSearchBO $filter)
    {
        ...
        $baseDataQuery = "
                 ...
                 where service_id = ? and seller_id = ? 
                  -- and spot_term = 1 
                 ";
...
//        $settingsService->getSettingsById("SellerPostMaster");
    ...
    public function handleMessagePostAdded(MessageBO $bo)
    {
        LOG::info("Handling Message Post Addition");
        NotificationService::notifyMessagePostCreated($bo);
        //0 indicates expire cache for all users.
        //  CacheControlService::markExpired(0, CacheControlService::BUYER_INBOUND_LEADS_OFFERS);
